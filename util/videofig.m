...
%   Jo鉶 Filipe Henriques, 2010
	...
	function check_int_scalar(a)
		assert(isnumeric(a) && isscalar(a) && isfinite(a) && a == round(a), ...%round四舍五入
			[upper(inputname(1)) ' must be a scalar integer number.']);%必须是标量整数，upper函数将小写字母转换为大写 如 A='aBc'; upper(A) 得 Ans= ABC 
	end
