function precisions = precision_plot(positions, ground_pos, title, show)
%PRECISION_PLOT
...
%   http://www.isr.uc.pt/~henriques/
    ...
    ground_pos = ground_pos(:,[2,1]) + (ground_pos(:,[4,3]) - ground_pos(:,[2,1])) / 2;
	
	
	precisions = zeros(max_threshold, 1);
	
	if size(positions,1) ~= size(ground_pos,1),
% 		fprintf('%12s - Number of ground truth frames does not match number of tracked frames.\n', title)
		...
		n = min(size(positions,1), size(ground_pos,1));
		positions(n+1:end,:) = [];
		ground_pos(n+1:end,:) = [];
	end
	
	%calculate distances to ground truth over all frames
	distances = sqrt((positions(:,1) - ground_pos(:,1)).^2 + ...
				 	 (positions(:,2) - ground_pos(:,2)).^2);
